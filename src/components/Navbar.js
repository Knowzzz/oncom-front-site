...
const Navbar = ({ isAuthenticated, color }) => {
  ...
  const buttonHoverColor =
    color === "white" ? "hover:bg-zinc-900" : "hover:bg-zinc-200";
  const borderColor = color === "white" ? "border-zinc-200" : "border-zinc-800";
  const logo =
    color === "white"
      ? "/images/logo_oncom_black.png"
      : "/images/logo_oncom.png";

  return (
    <nav
      className={`py-1 border ${borderColor} ${
        color === "white" ? "bg-white" : "bg-zinc-900"
      } shadow`}
    >
      <div className="container mx-auto flex flex-wrap justify-between items-center px-2">
        ...
        >
          <div
            className={`ml-16 flex flex-col space-y-8 md:space-y-0 md:flex-row md:space-x-8 ${textColor}`}
          >
            ...
            <Link
              to="/contact"
              className={`${textColor} underline-from-center`}
            >
              Contact
            </Link>
            <Link
